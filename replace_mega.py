...
for root, dirs, files in os.walk(base_dir):
    for file in files:
        if file.endswith('.html') and 'admin' not in root:
            ...
            # 정규식으로 GNB 영역 통째로 교체
            # <ul class="b2b-nav"> ... </ul> 부터 <div class="b2b-megamenu-inner"> ... </div> 까지
            pattern = re.compile(r'<ul class="b2b-nav">.*?<div class="b2b-megamenu-inner">.*?</div>\s*</div>', re.DOTALL)
            if pattern.search(content):
                # replacement
                # new_nav_html 안에 이스케이프해야 하는 부분이 거의 없으므로 바로 치환
                content = pattern.sub(new_nav_html.replace('\\', '\\\\') + '\n            </div>', content)
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(content)
                count += 1

print(f"총 {count}개의 HTML 파일에 새로운 6단 메가메뉴 구조를 이식했습니다.")
